        ...
        <div class="card" id="displaytable" style="width: 18rem;">
            ...
            <div class="card-body">
                ...
                if (isset($_POST['datePiick'])) {
                    ...
                    // print_r("my date" . $from);
                    // print_r("my date" . $to);
                    $stmt = $link->prepare("SELECT `r_id`, `title` from __rooms where `from` BETWEEN '" . $from .  "' AND `to` '" . $to .  "' ORDER by id DESC ");
                    $stmt->bind_param("ss", $from, $to);
                    $stmt->execute();
                    $newus = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
                    $newus = json_encode($newus, true);
                    print_r($newus['title']);
                    // exit();
                    // exit('success' . json_encode($newus['id']));
                }
                ?>
